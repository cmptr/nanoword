...
// Set difficulty to normal (0) always
...
for (let i = 0; i < 5; i++) {
  for (let j = 0; j < 5; j++) {
    ...
    grid[`row${i}`][j] = {
      ...
      acrossMarked: false,
      downMarked: false,
      opposite: grid[`row${4 - i}`][4 - j],
      above: i > 0 ? grid[`row${i - 1}`][j] : false,
      below: i < 4 ? grid[`row${i + 1}`][j] : false,
      prev: j > 0 ? grid[`row${i}`][j - 1] : false,
      next: j < 4 ? grid[`row${i}`][j + 1] : false,
    };
  }
}
...
for (let i = 4; i >= 0; i--) {
  for (let j = 4; j >= 0; j--) {
    ...
    currentCell.opposite = grid[`row${4 - i}`][4 - j];
    currentCell.next = j < 4 ? grid[`row${i}`][j + 1] : false;
    currentCell.below = i < 4 ? grid[`row${i + 1}`][j] : false;
  }
}
  ...
  for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 5; j++) {
      ...
      if (random < 0.3) {
        ...
        grid[`row${4 - i}`][4 - j].contents = "#";
        grid[`row${i}`][j].cell.style.backgroundColor = "black";
        grid[`row${4 - i}`][4 - j].cell.style.backgroundColor = "black";
...
processDisjointedWords(5, 5, "across");
processDisjointedWords(5, 5, "down");
...
for (let i = 0; i < 5; i++) {
  ...
  for (let j = 0; j < 5; j++) {
    ...
    // Down words will be processed in second pass
...
const validateWord = (result, comparison) => {
  ...
  return (
    !result.defs ||
    allWords.includes(result.word.toLowerCase()) ||
    result.word.length !== comparison.ref.length ||
    result.word.match(" ") ||
    result.word.match(/[0-9]/) ||
    result.word.match(/["!£$%&:-@/<>]/) ||
    disallowed.includes(result.word.toLowerCase())
  );
};
...
const createWordsAndClues = async () => {
  ...
  for (let i = 0; i < acrossWords.length; i++) {
    ...
    while (validateWord(retrievedWord, word)) {
      ...
      if (index >= shuffledWordList.length) {
        ...
        console.log("No suitable words found, trying generic query for length", len);
        queryParams = "?".repeat(len);
        retrievedWordList = await getNewWord(queryParams);
        
        if (retrievedWordList.length === 0) {
          // Last resort: use a simple word based on length
          const fallbackWords = {
          ...
          if (count > 0) {
            ...
            while (validateWord(randomDownWord, downWord)) {
              if (index > shuffledDownWordList.length) {
                // Try with a more generic query for down words
                console.log("No suitable down words found, trying generic query for length", downWord.ref.length);
                qParams = "?".repeat(downWord.ref.length);
                ...
                if (downWordList.length === 0) {
                  ...
                  const fallback = fallbackWords[downWord.ref.length] || ["word"];
                  const randomFallback = fallback[Math.floor(Math.random() * fallback.length)];
    ...
    for (let i = 0; i < allDownWords.length; i++) {
      ...
      if (current.ref[0].contents === "") {
        ...
        while (validateWord(finalNewWord, current)) {
          index++;
          if (index > shuffledNewWordList.length) {
            // Try with a more generic query for remaining words
            console.log("No suitable remaining words found, trying generic query for length", current.ref.length);
            query = "?".repeat(current.ref.length);
            newWordList = await getNewWord(query);
            ...
            if (newWordList.length === 0) {
              ...
              const fallback = fallbackWords[current.ref.length] || ["word"];
              const randomFallback = fallback[Math.floor(Math.random() * fallback.length)];
            ...
            shuffledNewWordList = newWordList.sort((a, b) => {
              return Math.random() > 0.5 ? 1 : -1;
            });
  ...
  // Start the timer when puzzle generation is complete
  startTimer();
};
...
const revealGrid = () => {
  ...
  // Stop the timer when puzzle is revealed
  stopTimer();
};
